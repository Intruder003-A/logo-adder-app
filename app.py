...
# Monkey-patch LocalSourcesWatcher to skip torch.classes
original_get_module_paths = local_sources_watcher.get_module_paths

def patched_get_module_paths(module):
    if module.__name__.startswith("torch.classes"):
        logging.info(f"Skipping module path extraction for {module.__name__}")
        return []
    return original_get_module_paths(module)

local_sources_watcher.get_module_paths = patched_get_module_paths
logging.info("Applied monkey-patch to Streamlit LocalSourcesWatcher for torch.classes")
...
try:
    ...
    if not existing_apps:
        try:
            ...
            firebase_admin.initialize_app(cred, name='logoadder_app')
        ...
        app_name = list(existing_apps.keys())[0] if existing_apps else 'logoadder_app'
        logging.info(f"Using existing Firebase app: {app_name}")
    db = firestore.client(app=firebase_admin.get_app(name='logoadder_app'))
...
class Config:
    ...
    LOGO_TRANSPARENCY = 0.9  # Adjusted for less transparency (more opacity)
